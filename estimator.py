...
class MHE:
    ...
    def __call__(self, y, args):
        ...
        if N == 1:
            ...
            if args['if_sigma'] and N > self.horizon:
...
def estimate(env, args):
    ...
    args['if_sigma'] = False
    # introduce model
    args['MODEL_SAVE'] = "save_model/better pi/model_v1.pt"
    # pre-trained noise generate model
    args['NOISE_SAVE'] = "save_model/better pi/noise.pt"
    args['SAVE_A1'] = "save_model/better pi/A1.pt"
    args['SAVE_B1'] = "save_model/better pi/B1.pt"
    args['SAVE_C1'] = "save_model/better pi/C1.pt"
    ...
    g0_guess = 1.2 * g0
    """MHE initial with x0"""
    # estimator = MHE(A, B, g0_guess, H, x_std, shift, model, args)
    ...
    for t in range(x_test.shape[0] - 1):
        y = x_test[t, :]
        x_est_pi[t, :] = estimator(y, args)   # this step can add a condition that t > MHE_horizon or just run from 0
    ...
    print("----------------without pi-----------------")
    args['if_sigma'] = False
    args['MODEL_SAVE'] = "save_model/nopi/model_v1.pt"
    args['SAVE_A1'] = "save_model/nopi/A1.pt"
    args['SAVE_B1'] = "save_model/nopi/B1.pt"
    args['SAVE_C1'] = "save_model/nopi/C1.pt"
    ...
    for i in range(3):  # 行索引
        for j in range(3):  # 列索引
            ...
            # axs[i, j].plot(x_est2[:, i * 3 + j], label='Adaptive Weight Matrices', color=color1, linewidth=2)
            # axs[i, j].plot(x_est[:, i * 3 + j], '--', label='Fixed Weight Matrices', color=color2, linewidth=2)
            # axs[i, j].set_title(f'State {i * 3 + j + 1}')
